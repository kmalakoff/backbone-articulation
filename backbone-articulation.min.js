...
a[JSON.deserialize.TYPE_FIELD];e=0;for(g=JSON.deserialize.NAMESPACE_ROOTS.length;e<g;e++){f=JSON.deserialize.NAMESPACE_ROOTS[e];a:{var h=f,i=c,f=i.split(".");if(f.length===1)f=h instanceof Object&&h.hasOwnProperty(i)?h[i]:void 0;else{for(var i=void 0,j=0,l=f.length;j<l;){i=f[j];if(!(i in h))break;if(++j===l){f=h[i];break a}h=h[i];if(!h||!(h instanceof Object))break}f=void 0}}if(f)if(f.fromJSON)return f.fromJSON(a);else if(f.prototype&&f.prototype.parse)return c=new f,c.set?c.set(c.parse(a)):c.parse(a)}return null}};
...
this.models[0]instanceof Backbone.RelationalModel?_.each(this.models,function(a){Backbone.Relational.store.unregister(a);a.clear({silent:!0})}):_.each(this.models,function(a){a.clear({silent:!0})}));return k.apply(this,arguments)};var f=Backbone.Collection.prototype._onModelEvent;Backbone.Collection.prototype._onModelEvent=function(a,c,b,d){if(a=="destroy"){for(var e in this._previousAttributes)this._disownAttribute(e,this._previousAttributes[e]);for(e in this.attributes)this._disownAttribute(e,this.attributes[e])}return f.apply(this,
arguments)};if(Backbone.RelationalModel)Backbone.RelationalModel.prototype.toJSON=function(){if(this.isLocked())return this.id;this.acquire();var a=Backbone.Model.prototype.toJSON.call(this);_.each(this._relations,function(c){var b=a[c.key];c.options.includeInJSON===!0&&b&&typeof b==="object"?a[c.key]=_.isFunction(b.toJSON)?b.toJSON():b:_.isString(c.options.includeInJSON)?b?b instanceof Backbone.Collection?a[c.key]=b.pluck(c.options.includeInJSON):b instanceof Backbone.Model?a[c.key]=b.get(c.options.includeInJSON):
