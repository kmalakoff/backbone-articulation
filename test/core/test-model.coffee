...
test("Model: memory management clone() and destroy()", ->
  ...
  equal(CloneDestroy.instance_count, 4, '1 referenced instance + 3 in attributes')
  model.set(deserialized_attributes)
  equal(CloneDestroy.instance_count, 10, '1 referenced instance + 3 in deserialized_attributes + 3 in attributes + 3 in previous attributes')
